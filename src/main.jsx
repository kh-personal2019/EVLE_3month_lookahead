...
function App() {
  ...
  async function loadEverything() {
    try {
      ...
      const [incomingEvents, incomingCategories] = await Promise.all([
        calendarApi.listEvents(),
        calendarApi.listCategories()
      ]);

      const nextCategories = incomingCategories.length
        ? incomingCategories
        : DEFAULT_CATEGORIES;
      ...
      setStatus(
        `Loaded ${incomingEvents.length} event${incomingEvents.length === 1 ? '' : 's'} and ${nextCategories.length} categor${nextCategories.length === 1 ? 'y' : 'ies'}.`
      );
    } catch (error) {
  ...
  function startNewEvent(date = todayISO()) {
    const fallbackCategory = categoryNames.includes('IDT')
      ? 'IDT'
      : categoryNames[0] || 'IDT';
    ...
    try {
      if (selectedEvent) {
        ...
        setEvents((current) =>
          current
            .map((event) =>
              event.id === selectedEvent.id ? updated.event : event
            )
            .sort(sortEvents)
        );
        ...
        setEvents((current) =>
          [...current, created.event].sort(sortEvents)
        );
  ...
  async function addCategory(formEvent) {
    ...
    try {
      const created = await calendarApi.createCategory({
        name,
      ...
      setCategories((current) =>
        [
          ...current.filter((item) => item.name !== created.category.name),
          created.category
  ...
  async function updateCategory(originalName, updates) {
    ...
    try {
      ...
      setCategories((current) =>
        current
          .map((item) =>
            item.name === originalName ? updated.category : item
          )
          .sort((a, b) => a.name.localeCompare(b.name))
      );
      ...
      if (updates.name && updates.name !== originalName) {
        setEvents((current) =>
          current.map((event) =>
            event.category === originalName
              ? { ...event, category: updates.name }
              : event
          )
        );

        setSelectedCategories((current) =>
          current.map((categoryName) =>
            categoryName === originalName ? updates.name : categoryName
          )
        );
      }
  ...
  function toggleSelectedCategory(categoryName, checked) {
    if (checked) {
      setSelectedCategories((current) =>
    ...
    <div className="app-shell">
      ...
      <section className="toolbar" aria-label="Calendar controls">
        ...
        <div className="month-controls">
          <button onClick={() => setAnchorMonth(addMonths(anchorMonth, -1))}>
            Previous
          </button>

          <strong>
            {view === 'quarter'
              ? `${monthTitle(anchorMonth)} to ${monthTitle(addMonths(anchorMonth, 2))}`
              : monthTitle(anchorMonth)}
          </strong>
      ...
      <section className="filter-panel" aria-label="Category filters">
        ...
        >
          <form onSubmit={saveEvent} className="event-form">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setDraft({
                      ...
                      date: newStart,
                      endDate:
                        !draft.endDate || draft.endDate < newStart
                          ? newStart
                          : draft.endDate
                    });
                  }}
            ...
            <label className="check-row">
              <input
                type="checkbox"
                checked={Boolean(draft.tentative)}
                onChange={(event) =>
                  setDraft({
                    ...draft,
                    tentative: event.target.checked
                  })
                }
              />
              Tentative
...
function CategoryEditor({ category, onUpdate, onDelete, disabled }) {
  ...
  return (
    <div className="category-row">
      ...
      <button
        type="button"
        onClick={() => onUpdate(category.name, { name, color })}
        disabled={disabled}
      >
        Save
      </button>
...
function Modal({ title, children, onClose, large = false }) {
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <div className={large ? 'modal-card modal-large' : 'modal-card'}>
      ...
      <div className="days-grid">
        {days.map((day) => {
          ...
          const dayEvents = events.filter((event) =>
            eventOccursOn(event, iso)
          );

          const grouped = categories
            .map((category) => ({
              category,
              events: dayEvents.filter((event) => event.category === category.name)
            }))
            .filter((group) => group.events.length > 0);

          const uncategorized = dayEvents.filter(
            (event) =>
              !categories.some((category) => category.name === event.category)
          );
          ...
          return (
            ...
            >
              ...
              <div className="event-stack">
                {grouped.map((group) => (
                  <div
                    className="category-event-group"
                    key={group.category.name}
                  >
                    <span
                      className="category-inline-label"
                    ...
                    {group.events.map((event) => (
                      <span
                        key={event.id}
                        title={event.title}
                        className={`event-chip ${event.tentative ? 'tentative' : ''}`}
                        style={{
                          borderLeftColor:
